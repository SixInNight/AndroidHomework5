    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // todo 做网络请求
                apiService.regist(name, password, repassword).enqueue(new Callback<UserResponse>() {
                    @Override
                    public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                        if (response.body().user != null) {
                            Toast.makeText(RegisterActivity.this, "恭喜您注册成功！请确认：\n您的用户名是 " + response.body().user.name, Toast.LENGTH_SHORT).show();
                        }
                        else if (response.body() != null) {
                            Toast.makeText(RegisterActivity.this, "很遗憾注册失败……\n" + response.body().errorMsg, Toast.LENGTH_SHORT).show();
                        }
                    }
